...
class CbRFRepository : ICurrencySource, ICachedCurrencySource  {
    companion object {
    ...
    private inner class ValCurs(
        val ValCurs: ArrayList<Valute> = ArrayList(36)
    )
    ...
    private val client = SingletonOkHttpObject.currencyRepositoryHttpClient
    ...
    private fun requestValCurs(): List<Valute> {
        ...
        if (response.isSuccessful) {
            ...
            if (body != null) {
                ...
                val valCurs = ValCurs().apply {
                    ValCurs.add(Valute("RUB", 1, "1"))
                }
    ...
    override fun getLatestRate(from: ICurrency, to: ICurrency): Rate {
        ...
        if (fromCurs != null && toCurs != null) {
            return Rate(calcRate(fromCurs, toCurs))
        }
