...
import dev.andrew.rates.source.currency.RussianCentralBankRepository
...
class AppSettings(
    ...
    companion object {
        ...
        @JvmStatic
        private val DEFAULT_REPOSITORY_INFO = RussianCentralBankRepository.INFO
    }
    ...
    private fun getLastUsedCurrencyFirstFromStorage(): ICurrency {
        try {
            return getCurrencyByStoreIndex(0)
                ?: CurrencyBuilder.buildFiat("EUR", RussianCentralBankRepository.INFO.id)
        } catch (e: Exception) {
            Log.d(TAG, "getLastUsedCurrencyFirstFromStorage", e)
            return CurrencyBuilder.buildFiat("EUR", RussianCentralBankRepository.INFO.id)
        }
    }
    ...
    private fun getLastUsedCurrencySecondFromStorage(): ICurrency {
        try {
            return getCurrencyByStoreIndex(1)
                ?: CurrencyBuilder.buildFiat("USD", RussianCentralBankRepository.INFO.id)
        } catch (e: Exception) {
            Log.d(TAG, "getLastUsedCurrencySecondFromStorage", e)
            return CurrencyBuilder.buildFiat("USD", RussianCentralBankRepository.INFO.id)
        }
    }
