...
object CurrencyHelper {
    ...
    private val CURRENCY_ARRAY = arrayOf(
        ...
        LocalCurrency(code = "EURUSD", symbol="€$"),
        LocalCurrency(code = "ADENA", symbol="\uD83E\uDE99")
    )
    ...
    console.log(result)
     */
    private val FIAT_POPULARITY_ARRAY = arrayOf(
        "USD",
        "EUR",
        "GBP",
        "INR",
    ...
    fun sortFiatByPopularity(list: List<ICurrency>): List<ICurrency> {
        return list.sortedBy {
            FIAT_POPULARITY_ARRAY.indexOf(it.name)?.let { index ->
                if (index == -1) {
                    1000
                } else {
                    index
    ...
    @DrawableRes
    fun getCountyFlagRes(context: Context, currency: ICurrency): Int {
        val sysCurrency = Currency.getInstance(currency.name)
        val sysLocale = Locale(sysCurrency.currencyCode)
        val resource = context.resources.getIdentifier(sysLocale.country, "drawable", null)
        if (resource != Resources.ID_NULL) {
        }
        Log.d("CurrencyHelper", "sysCurrency=$sysCurrency")
        Log.d("CurrencyHelper", "sysLocale=$sysLocale")
        Log.d("CurrencyHelper", "resource=$resource")
        return 0
    }
}